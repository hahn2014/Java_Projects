...
public class GraphPanel extends JPanel {
	...
	ArrayList<Circle> nodeList = new ArrayList<Circle>();
	ArrayList<Edge> edgeList = new ArrayList<Edge>();
	ArrayList<ArrayList<Boolean>> test = new ArrayList<ArrayList<Boolean>>();
	boolean[][] be = new boolean[nodeList.size()][nodeList.size()];
	...
	public void addEdge(Circle newfirst, Circle newsecond, String newlabel) {
		...
		//reset it yo
		test.get(firstIndex).set(secondIndex, true);
		test.get(secondIndex).set(firstIndex, true);
		printDaMatrix();
	}
	public void addNode(int newx, int newy, String newlabel) {
		nodeList.add(new Circle(newx, newy, newlabel));
		test.add(new ArrayList<Boolean>());
		for (int i = 0; i < test.size() - 1; i++) {
			test.get(i).add(false);
		}
		for (int i = 0; i < test.size(); i++) {
			test.get(test.size() - 1).add(false);
		}
		printDaMatrix();
